...
### 🍚 饮食记录
- **记一餐**：直接写「2个鸡蛋、一杯牛奶、一碗米饭」，自动按食物库（70+ 常见食物）估算热量与蛋白质/碳水/脂肪，支持中文数字（两碗）、克数（200g鸡胸）、尾部写法（米饭300g）；识别不了的保留原文并提示
...
- **自定义周期计划**：天数金额完全自由（如「14 天 ¥800」「10 天 ¥500」），可多个并存；未开始/进行中/已结束三种状态分别展示，结束后一键「再来一期」；节奏超支时给出「每天控制在 ¥X 内」的拉回建议
...
```bash
npm test          # 单元测试（训练 47 + 饮食 36 + 预算 42，共 125 项断言）
